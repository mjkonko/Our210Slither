...
class MySlitherModel {
    ...
    void addSnake(int snakeID, String name, double x, double y, double wang, double ang, double sp, double fam, Deque<SnakeBodyPart> body, double boostModif) {
        synchronized (view.modelLock) {
            Snake newSnake = new Snake(snakeID, name, x, y, wang, ang, sp, fam, body, this, boostModif);
            if (snake == null) {
                snake = newSnake;
            }
